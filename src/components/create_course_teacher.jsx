  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-purple-50 px-4">
      <div className="bg-purple-100 p-8 rounded-2xl shadow-md w-full max-w-md text-center">
        ...
        />
        <div className="flex items-center justify-between bg-white border border-purple-500 rounded p-2 mt-4">
          <span className={`font-semibold  ${courseCode ? "text-purple-700" : "text-gray-400"} `}>
            {courseCode || "Course Code"}
          </span>
          <button onClick={handleCopy} className={`hover:text-purple-900 ${courseCode ? "text-purple-700" : "text-gray-400 cursor-not-allowed"}`} disabled={!courseCode}>
            <Copy size={20} />
          </button>
        </div>
        <button
